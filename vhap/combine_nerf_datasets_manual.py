...
class NeRFDatasetAssembler:
    def __init__(self, src_folders: List[Path], tgt_folder: Path, division_mode: Literal['random_single', 'random_group', 'last']='random_group'):
        ...
        if division_mode == 'random_single':
        ...
        elif division_mode == 'manual':
            # Manually specify test sequences: EMO-3-angry+sad and SEN-09-frown_events_bad
            # These correspond to indices 2 and 13 in the sequence list

            manual_test_sequences = ['SEN-01-cramp_small_danger', 
            ...
            # Find and remove the specified sequences from src_folders
            indices_to_remove = []
            for i, folder in enumerate(self.src_folders):
                folder_sequence = '_'.join(folder.name.split('_')[1:])  # Remove subject prefix
                if folder_sequence in manual_test_sequences:
                    self.src_folders_test.append(folder)
                    indices_to_remove.append(i)
            
            # Remove test folders from src_folders (in reverse order to maintain indices)
            for idx in sorted(indices_to_remove, reverse=True):
                self.src_folders.pop(idx)
            
            print(f"Manual test sequences selected: {[f.name for f in self.src_folders_test]}")
        else:
...
def main(
        ...
        tgt_folder: Path,
        division_mode: Literal['random_single', 'random_group', 'last', 'manual']='random_group',
    ):
    incomplete = False
